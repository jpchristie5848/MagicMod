...
public class ShockStatusEffect extends AbstractElementalStatusEffect{
    ...
    protected ShockStatusEffect() {
        super(StatusEffectCategory.HARMFUL, 0xeeff00, MagicElement.LIGHTNING, "shock");
    }
    ...
    public void onApplied(LivingEntity entity, int amplifier) {
        ...
        if(entity.hasStatusEffect(DiabolismEffects.MAP.get("wet")) || isInWater(entity) || isWearingMetal(entity)){
            damage *= 2;
            paralysisProbability *= 2;
        }
        entity.damage(entity.getDamageSources().magic(), damage);
        if (Math.random() < paralysisProbability / 100.0f){
            entity.addStatusEffect(new StatusEffectInstance(DiabolismEffects.MAP.get("paralysis"), 200, 50));
        }
    }
